...
## [exposure/vs-proxy] Architecture proxy VS Code : sous-domaine fixe + upstream dynamique
Cloudflare Universal SSL couvre `*.yoops.org` mais PAS `*.dev.yoops.org` (deux niveaux → `ERR_SSL_VERSION_OR_CIPHER_MISMATCH`). Solution : `vs-dev.yoops.org` (un niveau, couvert par `*.yoops.org`). Un seul sous-domaine pour tous les workspaces. Caddy résout l'upstream dynamiquement : forward_auth vers `/auth/caddy/verify-workspace` qui retourne `X-Workspace-Upstream: portal:{port}` ; le handler `vars` Caddy l'injecte dans `{http.vars.workspace_upstream}` (via `handle_response` dans `_forward_auth_handler`) ; `_ws_proxy_dynamic()` dial ce placeholder. Le cookie de session + `?folder=` permet de sélectionner le bon workspace. `COOKIE_DOMAIN=yoops.org` obligatoire pour que le cookie de session soit envoyé à `vs-dev.yoops.org`.
