...
- Sans Docker local, `TEST_DATABASE_URL=postgresql+asyncpg://postgres:devpodtests@192.168.10.250:55432/portal_tests` (le `pg-tests` de test1) dé-skippe ~560 tests : un skip n'est pas un succès. Base PARTAGÉE : jamais deux runs pytest DB simultanés (même en fond) — les `create_all`/`drop_all` des fixtures se percutent (« relation X does not exist » fantômes). Suites routes+DB : app minimale + `db_engine_pool` + AsyncClient/ASGITransport (modèle test_me), jamais `create_app()`+TestClient. Portail en local : le cookie de session porte `domain=.dev.yoops.org`, donc un jar curl sur `127.0.0.1` ne le rejoue pas — extraire `Set-Cookie` et le repasser en `-b`.
