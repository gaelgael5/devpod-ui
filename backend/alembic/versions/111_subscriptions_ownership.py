...
- **`host_ownership` / `host_guests`** portent la propriete d'une machine dediee
  et ses invites. `max_workspaces` y est la CAPACITE DE LA MACHINE, partagee
  entre l'owner et ses invites — ce n'est pas un quota individuel toutes
  machines confondues.
...
def upgrade() -> None:
    ...
    if "host_ownership" not in presentes:
        op.create_table(
            ...
            # NULL = illimite. Capacite de la machine, pas quota individuel.
