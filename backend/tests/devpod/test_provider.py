...
async def test_ensure_provider_uses_ssh_for_ssh_host(
    ...
    assert any("HOST=devops@192.168.1.40" in c for c in calls), f"Expected HOST=user@ip option, got: {calls}"
    assert not any("USER=" in c for c in calls), f"Unexpected USER option (not valid for SSH provider), got: {calls}"
