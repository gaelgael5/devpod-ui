...
from portal.routes.applications import ApplicationBody


def test_accepts_https_url_and_emoji_icon() -> None:
    app = ApplicationBody(name=" Doc ", url="https://doc.yoops.org", icon=" 📚 ")
    assert app.name == "Doc"
...
def test_rejects_non_http_urls(url: str) -> None:
    with pytest.raises(ValidationError):
        ApplicationBody(name="Doc", url=url)


def test_rejects_javascript_icon_url() -> None:
    with pytest.raises(ValidationError):
        ApplicationBody(name="Doc", url="https://x.io", icon="javascript://x")


def test_rejects_empty_or_long_name() -> None:
    with pytest.raises(ValidationError):
        ApplicationBody(name="   ", url="https://x.io")
    with pytest.raises(ValidationError):
        ApplicationBody(name="x" * 61, url="https://x.io")


def test_rejects_unknown_field() -> None:
    with pytest.raises(ValidationError):
        ApplicationBody(name="Doc", url="https://x.io", extra="nope")  # type: ignore[call-arg]
