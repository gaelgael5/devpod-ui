...
Le point vérifié en priorité : `max_workspaces` est la capacité de LA MACHINE,
partagée entre le propriétaire et ses invités — pas un quota individuel.
"""
...
def test_la_capacite_restante_se_calcule_sur_le_total_de_la_machine() -> None:
    assert capacite_restante(_ownership(max_workspaces=5), 3) == 2


def test_la_capacite_restante_ne_devient_jamais_negative() -> None:
    assert capacite_restante(_ownership(max_workspaces=5), 7) == 0


def test_une_capacite_absente_signifie_illimitee() -> None:
    assert capacite_restante(_ownership(max_workspaces=None), 42) is None
...
def test_le_proprietaire_cree_dans_la_capacite_restante() -> None:
    verifier_creation(_ownership(max_workspaces=5), [], {"alice": 4}, "alice")
...
def test_la_capacite_de_la_machine_est_partagee_avec_les_invites() -> None:
    ...
    with pytest.raises(QuotaDepasse, match="capacité de la machine"):
        verifier_creation(
            _ownership(max_workspaces=5), [_guest("bob")], {"alice": 3, "bob": 2}, "alice"
        )
...
def test_un_invite_sans_sous_allocation_consomme_le_reste() -> None:
    verifier_creation(_ownership(max_workspaces=5), [_guest("bob")], {"alice": 1, "bob": 3}, "bob")
...
def test_une_machine_illimitee_n_oppose_aucun_quota() -> None:
    verifier_creation(_ownership(max_workspaces=None), [], {"alice": 99}, "alice")
...
def test_les_places_du_proprietaire_sont_le_reste_de_la_machine() -> None:
    assert (
        places_pour(_ownership(max_workspaces=5), [_guest("bob")], {"alice": 1, "bob": 2}, "alice")
        == 2
    )
...
def test_les_places_sont_illimitees_sans_capacite_ni_allocation() -> None:
    assert places_pour(_ownership(max_workspaces=None), [_guest("bob")], {}, "bob") is None


def test_une_allocation_borne_meme_une_machine_illimitee() -> None:
    places = places_pour(
        _ownership(max_workspaces=None),
