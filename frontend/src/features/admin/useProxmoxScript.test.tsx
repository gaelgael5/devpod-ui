...
import { useExecuteScript } from './useProxmoxScript'

function streamHandler(onSignal: (signal: AbortSignal) => void) {
  return http.post('/admin/hypervisors/:name/execute', ({ request }) => {
    onSignal(request.signal)
    // Stream qui ne se termine jamais dans la fenêtre du test — simule un
    // script encore en cours d'exécution quand le composant se démonte.
...
describe('useExecuteScript', () => {
  it('résout logs + done quand le stream se termine normalement', async () => {
    server.use(
      http.post('/admin/hypervisors/:name/execute', () => new HttpResponse('==> ok\n', { status: 200 })),
    )
    const { result } = renderHook(() => useExecuteScript())
    await act(async () => {
      await result.current.execute('node1', {})
    })
  ...
  it(
    ...
    async () => {
      ...
      const { result, unmount } = renderHook(() => useExecuteScript())

      let executePromise: Promise<void>
      act(() => {
        executePromise = result.current.execute('node1', {})
      })
